...
export default function ProductDetailsPage() {
  ...
  return <div className="detail-page">
    ...
    <section className="action-bar"><button className="primary" onClick={() => addToCart(product)}>Add to cart</button><button className="buy" onClick={() => notify('Checkout started')}>Buy now</button><button onClick={() => navigate(`/chat?listingId=${product.id}`)} aria-label="Chat with seller"><Icon name="chat"/></button><button onClick={() => notify('Seller contact is available for subscribers')} aria-label="Call"><Icon name="phone"/></button></section>
